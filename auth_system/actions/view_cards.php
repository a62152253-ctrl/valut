...
$stmt = $conn->prepare(
    "SELECT uuid, type, favorite, created_at, updated_at
     FROM vault_entries WHERE user_id = ? AND type = 'card' ORDER BY $order_sql"
);
...
<title>Cards – Vaultly</title>
<link rel="stylesheet" href="../css/app.css">
<style>
.item-card-top{display:flex;align-items:flex-start;justify-content:space-between;margin-bottom:10px;}
.item-icon{width:42px;height:42px;border-radius:var(--r3);display:flex;align-items:center;justify-content:center;font-size:20px;flex-shrink:0;background:var(--red-s);}
...
.pass-toggle button{padding:10px 13px;background:var(--red-s);border:1px solid rgba(239,68,68,.22);color:#ef4444;border-radius:var(--r2);cursor:pointer;font-size:14px;}
...
<body>
<div class="layout">
<?php $active_nav = 'cards'; include '../includes/sidebar.php'; ?>
<div class="main">
    <div class="page-header">
        <div>
            <div class="page-title">💳 Cards</div>
            <div class="page-subtitle"><?php echo $total; ?> payment cards</div>
        </div>
        <div class="header-right">
            ...
            <button class="btn-primary" onclick="openAddModal()">＋ Add Card</button>
        </div>
    </div>
    ...
    <div class="items-grid stagger-in" id="itemsGrid">
        <?php if (count($entries) > 0): ?>
            ...
            <div class="item-card card-glow"
                 ...
                 data-type="card" data-search="">
                <div class="item-card-top">
                    <div style="display:flex;align-items:center;gap:12px;">
                        <div class="item-icon">💳</div>
                        <div>
                            <span class="type-badge card">card</span>
                ...
                <div class="item-actions">
                    ...
                    <button class="btn-sm btn-copy" onclick="copyCard('<?php echo $entry['uuid']; ?>')">Copy #</button>
        ...
        <?php else: ?>
            <div style="grid-column:1/-1;">
                <div class="empty-state">
                    ...
                    <p>Store payment card details securely.</p>
                    <button class="btn-primary" onclick="openAddModal()">＋ Add Card</button>
                </div>
            </div>
        <?php endif; ?>
...
<div id="vaultLockOverlay" class="vault-lock-overlay">
    <div class="vault-lock-box">
        ...
        <h2>Unlock Vault</h2>
        <p>Enter your master password to access cards.</p>
...
<div id="itemModal" class="modal">
    <div class="modal-box">
        ...
        <form id="itemForm">
            ...
            <div class="form-group"><label>Card Name / Label</label><input type="text" id="editTitle" placeholder="e.g. Visa Business" required></div>
            <div class="form-group"><label>Cardholder Name</label><input type="text" id="editCardholderName" placeholder="Name on card"></div>
            <div class="form-group"><label>Card Number</label>
                <div class="pass-toggle">
                    <input type="password" id="editCardNumber" placeholder="•••• •••• •••• ••••" maxlength="19">
                    <button type="button" onclick="toggleField('editCardNumber')" aria-label="Show/hide">👁</button>
                </div>
            </div>
            <div class="form-group"><label>Expiry</label><input type="text" id="editExpiry" placeholder="MM/YY" maxlength="5"></div>
            <div class="form-group"><label>CVV</label>
                <div class="pass-toggle">
                    <input type="password" id="editCvv" placeholder="•••" maxlength="4">
                    <button type="button" onclick="toggleField('editCvv')" aria-label="Show/hide">👁</button>
                </div>
            </div>
            <div class="form-group"><label>Notes</label><textarea id="editNotes" placeholder="Additional info…"></textarea></div>
            <div class="modal-actions">
                <button type="submit" class="btn-save" id="saveBtn">Save Card</button>
...
async function loadEntries(){
    ...
    for(const row of(data.entries||[])){if(row.type!=='card')continue;try{vault.entries[row.uuid]=await CryptoEngine.decrypt(vault.key,row.iv,row.encrypted_data);}catch{}}
    applyDecryptedData();
}
...
function applyDecryptedData(){
    for(const[uuid,d]of Object.entries(vault.entries)){
        ...
        if(metaEl){
            const last4=d.cardNumber?(d.cardNumber.replace(/\s/g,'').slice(-4)):'';
            metaEl.innerHTML=(d.cardholderName?`👤 ${esc(d.cardholderName)}`:'')+(last4?` •••• ${esc(last4)}`:'');
        }
    }
...
function openAddModal(){
    ...
    document.getElementById('modalTitle').textContent='Add Card';document.getElementById('saveBtn').textContent='Save Card';
...
function openEditModal(uuid){
    ...
    document.getElementById('modalTitle').textContent='Edit Card';document.getElementById('saveBtn').textContent='Save Changes';
    document.getElementById('editUuid').value=uuid;document.getElementById('editTitle').value=d.title||'';
    document.getElementById('editCardholderName').value=d.cardholderName||'';document.getElementById('editCardNumber').value=d.cardNumber||'';
    document.getElementById('editExpiry').value=d.expiry||'';document.getElementById('editCvv').value=d.cvv||'';
...
document.getElementById('itemForm').addEventListener('submit',async function(e){
    ...
    const entry={title:document.getElementById('editTitle').value,cardholderName:document.getElementById('editCardholderName').value,cardNumber:document.getElementById('editCardNumber').value,expiry:document.getElementById('editExpiry').value,cvv:document.getElementById('editCvv').value,notes:document.getElementById('editNotes').value};
    ...
    try{
        ...
        const result=await(await fetch('../api/vault.php',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({action:uuid?'update':'create',uuid:uuid||undefined,type:'card',encrypted_data:encData,iv,favorite:0})})).json();
        if(result.error)throw new Error(result.error);
        showToast(uuid?'✓ Card updated!':'✓ Card saved!');closeModal();await loadEntries();
    }catch(err){showToast('Error: '+err.message,'error');}finally{btn.disabled=false;btn.textContent=uuid?'Save Changes':'Save Card';}
});
...
async function deleteItem(uuid,btn){if(!confirm('Delete this card?'))return;const d=await(await fetch('../api/vault.php',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({action:'delete',uuid})})).json();if(d.ok){const c=btn.closest('.item-card');c.style.opacity='0';setTimeout(()=>{c.remove();showToast('🗑 Deleted');},250);}else showToast('Error: '+(d.error||'Unknown'),'error');}
function copyCard(uuid){if(!vault.key){showToast('Unlock vault first','error');return;}const n=vault.entries[uuid]?.cardNumber?.replace(/\s/g,'')||'';if(!n){showToast('No card number','error');return;}navigator.clipboard.writeText(n).then(()=>showToast('✓ Card number copied'));}
