...
// Only login-type entries; no encrypted_data on server
$stmt = $conn->prepare(
    "SELECT uuid, type, favorite, created_at, updated_at
     FROM vault_entries WHERE user_id = ? AND type = 'login' ORDER BY $order_sql"
);
...
$count_stmt = $conn->prepare("SELECT COUNT(*) as cnt FROM vault_entries WHERE user_id = ? AND type = 'login'");
$count_stmt->bind_param('i', $user_id);
$count_stmt->execute();
$total = (int)($count_stmt->get_result()->fetch_assoc()['cnt'] ?? 0);
$count_stmt->close();
?>
<!DOCTYPE html>
<html lang="en">
...
<title>Password Manager – Vaultly</title>
<link rel="stylesheet" href="../css/app.css">
<style>
.item-card-top { display:flex; align-items:flex-start; justify-content:space-between; margin-bottom:10px; }
.item-icon { width:42px; height:42px; border-radius:var(--r3); display:flex; align-items:center; justify-content:center; font-size:20px; flex-shrink:0; background:var(--a-dim); }
...
.pass-toggle button { padding:10px 13px; background:var(--a-dim); border:1px solid rgba(99,102,241,.22); color:var(--a3); border-radius:var(--r2); cursor:pointer; font-size:14px; }
...
<body>
<div class="layout">
<?php $active_nav = 'password_manager'; include '../includes/sidebar.php'; ?>

<div class="main">
    <div class="page-header">
        <div>
            <div class="page-title">Password Manager</div>
            <div class="page-subtitle"><?php echo $total; ?> login credentials</div>
        </div>
        <div class="header-right">
            <div class="search-bar">
                <span class="search-icon">🔍</span>
                <input type="text" id="searchInput" placeholder="Search logins…">
            </div>
            <button class="btn-primary" onclick="openAddModal()">＋ Add Login</button>
        </div>
    </div>
    ...
    <div class="items-grid stagger-in" id="itemsGrid">
        <?php if (count($entries) > 0): ?>
            ...
            <div class="item-card card-glow"
                 ...
                 data-type="login" data-search="">

                <div class="item-card-top">
                    <div style="display:flex;align-items:center;gap:12px;">
                        <div class="item-icon">🔐</div>
                        <div>
                            <span class="type-badge login">login</span>
                ...
                <div class="item-actions">
                    ...
                    <button class="btn-sm btn-copy"  onclick="copyPwd('<?php echo $entry['uuid']; ?>')">Copy pwd</button>
        ...
        <?php else: ?>
            <div style="grid-column:1/-1;">
                <div class="empty-state">
                    ...
                    <h3>No logins yet</h3>
                    <p>Save your website credentials here.</p>
                    <button class="btn-primary" onclick="openAddModal()">＋ Add Login</button>
                </div>
            </div>
        <?php endif; ?>
...
<div id="itemModal" class="modal">
    <div class="modal-box">
        <div class="modal-head">
            <h2 id="modalTitle">Edit Login</h2>
            <button class="modal-close" onclick="closeModal()">✕</button>
        </div>
        <form id="itemForm">
            <input type="hidden" id="editUuid" value="">
            <div class="form-group"><label>Title / Site</label><input type="text" id="editTitle" placeholder="e.g. Gmail" required></div>
            <div class="form-group"><label>Username / Email</label><input type="text" id="editUsername" placeholder="username or email"></div>
            <div class="form-group">
                <label>Password</label>
                <div class="pass-toggle">
                    <input type="password" id="editPassword" placeholder="password">
                    <button type="button" onclick="togglePwd()" aria-label="Show/hide">👁</button>
                </div>
            </div>
            <div class="form-group"><label>Website URL</label><input type="url" id="editUrl" placeholder="https://example.com"></div>
...
function openAddModal() {
    ...
    document.getElementById('modalTitle').textContent='Add Login';
    document.getElementById('saveBtn').textContent='Add Login';
...
function openEditModal(uuid) {
    ...
    document.getElementById('modalTitle').textContent='Edit Login';
...
function togglePwd() { const f=document.getElementById('editPassword'); f.type=f.type==='password'?'text':'password'; }

document.getElementById('itemForm').addEventListener('submit', async function(e) {
    e.preventDefault();
    if (!vault.key) { showToast('Vault is locked','error'); return; }
    const uuid  = document.getElementById('editUuid').value;
    const entry = { title:document.getElementById('editTitle').value, username:document.getElementById('editUsername').value, password:document.getElementById('editPassword').value, url:document.getElementById('editUrl').value, notes:document.getElementById('editNotes').value };
    ...
    try {
        ...
        const result = await (await fetch('../api/vault.php',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({action:uuid?'update':'create',uuid:uuid||undefined,type:'login',encrypted_data:encData,iv,favorite:0})})).json();
        if (result.error) throw new Error(result.error);
        showToast(uuid?'✓ Updated!':'✓ Added!');
        closeModal();
        await loadEntries();
    } catch(err) { showToast('Error: '+err.message,'error'); }
    finally { btn.disabled=false; btn.textContent=uuid?'Save Changes':'Add Login'; }
});
...
async function deleteItem(uuid, btn) {
    if (!confirm('Delete this login? Cannot be undone.')) return;
...
function copyPwd(uuid) {
    if (!vault.key) { showToast('Unlock vault first','error'); return; }
    const text = vault.entries[uuid]?.password||'';
    if (!text) { showToast('No password to copy','error'); return; }
    navigator.clipboard.writeText(text).then(()=>showToast('✓ Password copied'));
}

function filterCards(q) {
    document.querySelectorAll('.item-card').forEach(c => { c.style.display = (!q||c.dataset.search.includes(q))?'':'none'; });
}
